...
ZEST_SUITE(serde_json_schema) {
    ...
    ZEST_CASE(schema_agrees_with_encoder_on_nan_null) {
        const auto encoded =
            json::to_string<nan_null_config>(std::numeric_limits<double>::quiet_NaN());
        ASSERT(encoded.has_value());
        EXPECT(*encoded == "null");
    ...
    ZEST_CASE(schema_agrees_with_encoder_on_nan_string) {
        const auto encoded =
            json::to_string<nan_string_config>(std::numeric_limits<float>::infinity());
        ASSERT(encoded.has_value());
