...
ZEST_SUITE(language_protocol_model){
...
ZEST_CASE(literal_selects_variant_alternative) {
    ...
    ASSERT(edit->document_changes.has_value());
...
ZEST_CASE(inherited_properties_are_members) {
    ...
    ASSERT(params->work_done_token.has_value());
...
ZEST_CASE(nested_structure_round_trip) {
    ...
    ASSERT(symbol->children.has_value());
...
ZEST_CASE(lsp_any_is_a_dynamic_value) {
    ...
    ASSERT(command->arguments.has_value());
