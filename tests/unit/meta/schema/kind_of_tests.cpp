...
ZEST_CASE(enums) {
    ...
    STATIC_EXPECT(kind_of<fx::SmallEnum>() == type_kind::enumeration);
}

ZEST_CASE(compounds) {
    STATIC_EXPECT(kind_of<std::vector<int>>() == type_kind::array);
    STATIC_EXPECT(kind_of<std::set<int>>() == type_kind::set);
    STATIC_EXPECT(kind_of<std::map<std::string, int>>() == type_kind::map);
    STATIC_EXPECT(kind_of<std::optional<int>>() == type_kind::optional);
    STATIC_EXPECT(kind_of<std::unique_ptr<int>>() == type_kind::pointer);
    STATIC_EXPECT(kind_of<std::shared_ptr<int>>() == type_kind::pointer);
    STATIC_EXPECT(kind_of<std::variant<int, std::string>>() == type_kind::variant);
    STATIC_EXPECT(kind_of<std::tuple<int, float>>() == type_kind::tuple);
    STATIC_EXPECT(kind_of<std::pair<int, std::string>>() == type_kind::tuple);
