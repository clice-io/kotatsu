...
ZEST_CASE(option_declaration_has_expected_shape_and_default_assignment) {
    DeclOpt opt{};
...
ZEST_CASE(option_into_assigns_values_by_option_kind) {
    ...
    EXPECT(!custom_scalar_ok);
    EXPECT(custom_scalar);
    ...
    EXPECT(!custom_vector_ok);
    EXPECT(custom_vector);
