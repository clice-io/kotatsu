...
#define KOTA_EXPECTED_TRY_V(decl, expr)                                                             \
    auto KOTA_CONCAT_(_eventide_try_tmp_, __LINE__) = (expr);                                       \
    if(!KOTA_CONCAT_(_eventide_try_tmp_, __LINE__))                                                 \
        return std::unexpected(std::move(KOTA_CONCAT_(_eventide_try_tmp_, __LINE__)).error());      \
    decl = std::move(*KOTA_CONCAT_(_eventide_try_tmp_, __LINE__))
