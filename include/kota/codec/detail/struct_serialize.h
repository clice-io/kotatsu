...
auto serialize_slot_value(S& s, const V& value) -> std::expected<void, E> {
    ...
    // No behavior: for tagged variants, preserve the annotation wrapper
    if constexpr(is_specialization_of<std::variant, std::remove_cvref_t<V>> &&
                 tuple_any_of_v<Attrs, meta::is_tagged_attr>) {
        // The caller should have passed the annotated field.value() for tagged variants.
        // This path handles the case where raw variant reaches here with tagged attrs.
        auto r = codec::serialize(s, value);
        if(!r) {
            return std::unexpected(r.error());
        }
        return {};
